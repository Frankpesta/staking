...
import { Wallet, TrendingUp, ArrowUp, Lock } from "lucide-react";
...
export default function DashboardPage() {
  ...
  // Get accumulated ROI from active staking pools
  const accumulatedRoi = useQuery(
    api.staking.getUserTotalAccumulatedRoi,
    user?._id ? { userId: user._id } : "skip"
  );
  ...
  // Calculate ROI value in USD
  const roiValue = usePortfolioValue(
    accumulatedRoi
      ? {
          availableBalance: accumulatedRoi,
          stakedBalance: {},
          depositBalance: {},
        }
      : undefined
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {balance === undefined ? (
        ...
        <StaggerChildren className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Return on Staking</CardTitle>
            ...
            <CardContent>
              <div className="text-xl sm:text-2xl font-bold wrap-break-word">
                {roiValue.isLoading ? (
                  ...
                  `$${roiValue.totalAvailable.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                )}
              </div>
              <p className="text-xs text-muted-foreground">
